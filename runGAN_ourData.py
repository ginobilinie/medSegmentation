...
parser.add_argument("--gpuID",default='6',type=str, help="the GPU ID")
...
parser.add_argument("--given_weight", type=arg_as_list, default=[1,4,8,6], help="the given weight for each organ (bg included)")
...
parser.add_argument("--test_step_sz", type=arg_as_list, default=[4,6,6], help="the step size at testing one subject")
...
parser.add_argument("--iter_size", type=int, default=4, help="number of iterations(actually, it is batches) to update the network parameters")
...
parser.add_argument("--resume", default="/shenlab/lab_stor5/dongnie/Seg3D_wdice_wce_viewExp_resEnhance_fullAD0p1_0224_ourData_30000.pt", type=str, help="Path to checkpoint (default: none)")
parser.add_argument("--resume_netD", default="/shenlab/lab_stor5/dongnie/Seg3D_wdice_wce_viewExp_resEnhance_fullAD0p1_0224_ourData_netD_30000.pt", type=str, help="Path to checkpoint (default: none)")
parser.add_argument("--start_epoch", default=1, type=int, help="Manual epoch number (useful on restarts)")
parser.add_argument("--threads", type=int, default=1, help="Number of threads for data loader to use, Default: 1")
parser.add_argument("--momentum", default=0.9, type=float, help="Momentum, Default: 0.9")
parser.add_argument("--weight-decay", "--wd", default=1e-4, type=float, help="weight decay, Default: 1e-4")
parser.add_argument("--pretrained", default="", type=str, help="path to pretrained model (default: none)")
parser.add_argument("--prefixModelName", default="/shenlab/lab_stor5/dongnie/Seg3D_wdice_wce_viewExp_resEnhance_fullAD0p1_0224_ourData_", type=str, help="prefix of the to-be-saved model name")
#parser.add_argument("--prefixModelName", default="/home/dongnie/Desktop/myPyTorch/pytorch-SRResNet23D/SegCha_3D_wce_wdice_viewExp_resEnhance_lrdcr_fullAd_0211_ourData_", type=str, help="prefix of the to-be-saved model name")
parser.add_argument("--prefixPredictedFN", default="preSub1_3D_wdice_wce_viewExp_resEhance_fullAD0p1_0224_ourData_", type=str, help="prefix of the to-be-saved predicted filename")
...
# parser.add_argument("--path_test",default='/home/dongnie/warehouse/pelvicSeg/mrs_data',type=str, help="the path for the testing nii.gz files")
# parser.add_argument("--path_patients_h5",default='/home/dongnie/warehouse/pelvicSeg/pelvic3DSegH5',type=str, help="the path for the training hdf5 files")
# parser.add_argument("--path_patients_h5_test",default='/home/dongnie/warehouse/pelvicSeg/pelvic3DSegH5Test',type=str, help="the path for the testing hdf5 files")
parser.add_argument("--path_test",default='/shenlab/lab_stor5/dongnie/pelvic/',type=str, help="the path for the testing nii.gz files")
parser.add_argument("--path_patients_h5",default='/shenlab/lab_stor5/dongnie/pelvic/pelvic3DSegH5',type=str, help="the path for the training hdf5 files")
parser.add_argument("--path_patients_h5_test",default='/shenlab/lab_stor5/dongnie/pelvic/pelvic3DSegH5Test',type=str, help="the path for the testing hdf5 files")
...
#                 elif opt.NDim==3:
    ...
    #             outputG = outputG.data.max(1)[1]
            ...
            if opt.isSoftmaxLoss:
                if opt.isNetDFullyConv and opt.isConfidenceRegionAware:
                    lossG_G_main = criterion_WCEND(outputG,torch.squeeze(labels),confRegion) 
                elif opt.isSampleImportanceFromAd:
                    lossG_G_main = (1+adImportance) * criterion_CEND(outputG,torch.squeeze(labels)) 
                else:
                    lossG_G_main = criterion_CEND(outputG,torch.squeeze(labels)) 
                if opt.isDeeplySupervised:
                    lossG_G_path1 = opt.lambda_ds1*criterion_CEND(outputG_path1,torch.squeeze(labels)) 
                    lossG_G_path2 = opt.lambda_ds2*criterion_CEND(outputG_path2,torch.squeeze(labels)) 
    ...
    #             criterion_dice = myWeightedDiceLoss4Organs(organIDs=[0,1,2,3], organWeights=[1,4,8,6])
                if opt.isSampleImportanceFromAd:
                    loss_dice_main = (1+adImportance) * criterion_dice(outputG,torch.squeeze(labels))
                else:
                    loss_dice_main = criterion_dice(outputG,torch.squeeze(labels))
                if opt.isDeeplySupervised:
                    loss_dice_path1 = opt.lambda_ds1*criterion_dice(outputG_path1,torch.squeeze(labels)) 
                    loss_dice_path2 = opt.lambda_ds2*criterion_dice(outputG_path2,torch.squeeze(labels)) 
    ...
    #             loss_dice.backward(retain_graph=True) #compute gradients for dice loss
            ...
            if opt.isFocalLoss:
                lossG_focal = lossG_focal + criterion_focal(outputG,torch.squeeze(labels))
...
#         print 'type of lossD: ',type(lossD),' type of lossD/opt.iter_size: ', type(lossD/opt.iter_size)
        if opt.isAdLoss:
            showTestStatistics(netG, lossG_G/opt.iter_size, loss_dice/opt.iter_size, lossG_D/opt.iter_size, lossG_focal/opt.iter_size, lossG_contour/opt.iter_size, lossG_Reg/opt.iter_size, lossD/opt.iter_size, iter,labeled=labeled, netD=netD)
        else:
            showTestStatistics(netG, lossG_G/opt.iter_size, loss_dice/opt.iter_size, lossG_D/opt.iter_size, lossG_focal/opt.iter_size, lossG_contour/opt.iter_size, lossG_Reg/opt.iter_size, lossD/opt.iter_size, iter,labeled=labeled)
