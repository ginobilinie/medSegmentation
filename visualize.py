...
'''
    Visualize the model (arch).
    the main code is from:
    https://github.com/ginobilinie/pytorchviz
'''
...
#     g.view()
    ...
    inputs = Variable(torch.randn(1, 1, 16, 64, 64))

# netG = ResSegNet(isRandomConnection=True, isSmallDilation=True, isSpatialDropOut=True, dropoutRate=0.2)
    netG = HRResUNet_DS(1, 2, isRandomConnection=False,
                                isSmallDilation=True, isSpatialDropOut=True,
                                dropoutRate=0.2, TModule='HR', FModule=True, nd=3)
    modelPath = 'preSub_wdice_wce_dilation_1028_50000.nii.gz'
#     netG.load_state_dict(torch.load(modelPath))
    y = netG(inputs)
    # make_dot(y, params=dict(netG.named_parameters()))

    g = make_dot(y, params=dict(netG.named_parameters()))
    # # g = make_dot(y, params=dict(netG.named_parameters()))
    #
    # g.save('HRResSEUNet_DS.gv')
    g.view()
