...
parser.add_argument("--given_weight", type=arg_as_list, default=[1,6,8,8], help="the given weight for each organ (bg included)")
parser.add_argument("--given_ids", type=arg_as_list, default=[0,1,2,3], help="the given organ id for each organ (bg included)")
parser.add_argument("--input_sz", type=arg_as_list, default=[3,168,112], help="the input patch size of list")
parser.add_argument("--output_sz", type=arg_as_list, default=[1,168,112], help="the output patch size of list")
...
parser.add_argument("--isSemanticGuidance", action="store_true", help="Do we use semantic guidance from high layers for shallow layer feature learning?", default=False)
parser.add_argument("--isLongConcatConnection", action="store_true", help="is the long skip connection concatenated?", default=False)
parser.add_argument("--isSimpleFCN", action="store_true", help="is the simple FCN?", default=True)
parser.add_argument("--isAttentionConcat", action="store_true", help="do we use attention based concatenation?", default=False)
parser.add_argument("--isWiderNet", action="store_true", help="do we use widder network?", default=False)
...
parser.add_argument("--how2NetNorm", default="BN", type=str, help="how to do the layer normalization for the neural network, BN, GN ..")
parser.add_argument("--how2normalize", type=int, default=4, help="how to normalize the data")
parser.add_argument("--batchSize", type=int, default=10, help="training batch size")
parser.add_argument("--iter_size", type=int, default=1, help="number of iterations(actually, it is batches) to update the network parameters")
...
parser.add_argument("--prefixModelName", default="/home/dongnie/Desktop/myPyTorch/pytorch-SRResNet23D/SegPelvic_2p5D_geneDice_mce_lr2e3_lrdce_viewExp_FCN_0312_", type=str, help="prefix of the to-be-saved model name")
# parser.add_argument("--prefixModelName", default="/shenlab/lab_stor/dongnie/challengeData/modelFiles/SegCha_3D_geneDice_mce_lr2e3_lrdce_viewExp_DS_SemGuided_0308_", type=str, help="prefix of the to-be-saved model name")
parser.add_argument("--prefixPredictedFN", default="preSub1_segPelvic_2p5D_geneDice_mce_lr2e3_lrdce_viewExp_FCN_0312_", type=str, help="prefix of the to-be-saved predicted filename")
parser.add_argument("--isTestonAttentionRegion", action="store_true", help="Test on the attention region?", default=True)

parser.add_argument("--test_input_file_name",default='img1_crop.nii.gz',type=str, help="the input file name for testing subject")
parser.add_argument("--test_gt_file_name",default='img1_label_nie_crop.nii.gz',type=str, help="the ground-truth file name for testing subject")
...
parser.add_argument("--path_test",default='/home/dongnie/warehouse/pelvicSeg/mrs_29/',type=str, help="the path for the testing nii.gz files")
parser.add_argument("--path_patients_h5",default='/home/dongnie/warehouse/pelvicSeg/pelvicSegRegContourBatchH5',type=str, help="the path for the training hdf5 files")
parser.add_argument("--path_patients_unlabeled_h5",default='...',type=str, help="the path for the training unlabeled hdf5 files")
parser.add_argument("--path_patients_h5_test",default='/home/dongnie/warehouse/pelvicSeg/pelvicSegRegContourH5Test',type=str, help="the path for the testing hdf5 files")
...
#     running_loss = 0
    ...
    if opt.isSegReg:
        ...
        netG = WDHRResSegNet_DS(opt.in_channels, opt.out_channels, nd=opt.NDim, isRandomConnection=opt.isResidualEnhancement,isSmallDilation=opt.isViewExpansion, isSpatialDropOut=opt.isSpatialDropOut,dropoutRate=opt.dropoutRate)
    elif opt.isDeeplySupervised and opt.isSemanticGuidance:
        netG = SemGResSegNet_DS(opt.in_channels, opt.out_channels, nd=opt.NDim,
                              isRandomConnection=opt.isResidualEnhancement, isSmallDilation=opt.isViewExpansion,
                              isSpatialDropOut=opt.isSpatialDropOut, dropoutRate=opt.dropoutRate, how2norm=opt.how2NetNorm)
    ...
    elif opt.isSimpleFCN:
        netG = FCN(opt.in_channels, opt.out_channels, nd=opt.NDim)
    else:
...
#     softmax2d = nn.Softmax2d()
    ...
    if opt.isFocalLoss:
        criterion_focal = FocalLossMultiCategory(gamma = opt.focal_gamma, weight = given_weight)
...
#     running_loss = 0.0
    ...
    for iter in range(opt.start_epoch, opt.numofIters+1):
        ...
        for m in range(0,opt.iter_size):
            ...
            # print 'inputs.size: ',inputs.shape,' labels.size: ',labels.shape
            ...
            if opt.isAdLoss:
                ...
                elif opt.isContourLoss and opt.isDeeplySupervised:
                    outputG, outputG_path1, outputG_path2, outputG_contour = netG(inputs)
                elif opt.isContourLoss:    
...
#                 elif opt.NDim==3:
    ...
    #             outputG = outputG.data.max(1)[1]
                ...
                #compute the fake regions, here we already use data, so it is not variable any more
                averProbTensor = 1 - outputD.data[0].cpu()
                #and we pay more attention of fake regions
                if opt.isNetDFullyConv and opt.isConfidenceRegionAware:
                    ...
                    # print 'confRegion.shape: ',confRegion.shape
            ...
            if opt.isSegReg:
                ...
                outputG,outputG_contour = netG(inputs)
            elif opt.isDeeplySupervised:
                outputG, outputG_path1, outputG_path2 = netG(inputs)
            else:
            ...
            if opt.isSoftmaxLoss:
                if opt.isNetDFullyConv and opt.isConfidenceRegionAware:
                    ...
                    lossG_G_main = opt.lambda_WCE * criterion_CEND(outputG,torch.squeeze(labels,dim=1))
    ...
    #             loss_dice.backward(retain_graph=True) #compute gradients for dice loss
            ...
            if opt.isContourLoss:
                lossG_contour = lossG_contour + criterion_BCE2D(outputG_contour,contours)
...
#             print 'running loss is ',running_loss
    ...
    if iter%opt.showTestPerformanceEvery==0: #test one subject  
        ...
        # print 'type(lossG_G): ',type(lossG_G)
        if opt.isDiceLoss or opt.isGeneDiceLoss:
            # print 'outputG: ',outputG.shape,' labels: ',labels.shape
            loss_dice = criterion_dice(outputG,torch.squeeze(labels,dim=1))
            # print 'lossG_G and loss_dice are %.2f and %.2f respectively.' % (lossG_G.data[0], loss_dice.data[0])
            print 'lossG_G and loss_dice are %.2f and %.2f respectively.' % (lossG_G.data, loss_dice.data)
        else:
