...
parser.add_argument("--input_sz", type=arg_as_list, default=[3,256,256], help="the input patch size of list")
parser.add_argument("--output_sz", type=arg_as_list, default=[1,256,256], help="the output patch size of list")
...
# parser.add_argument("--modelPath", default="/shenlab/lab_stor5/dongnie/pelvic/models/Seg3D_wdice_wce_viewExp_resEnhance_fullAD0p05_0224_ourData_70000.pt", type=str, help="prefix of the to-be-saved model name")
parser.add_argument("--modelPath", default="/shenlab/lab_stor5/dongnie/pelvic/models/SegOurData_2p5D_geneDice_mce_lr2e3_lrdceLimit_viewExp_ResSeg_DS_SemGuided_fullAD_lambda0p001_0513_100000.pt", type=str, help="prefix of the to-be-saved model name")
# parser.add_argument("--netDModelPath", default="/shenlab/lab_stor5/dongnie/pelvic/models/Seg3D_wdice_wce_viewExp_resEnhance_fullAD0p05_0224_ourData_netD_70000.pt", type=str, help="prefix of the to-be-saved model name")
parser.add_argument("--netDModelPath", default="/shenlab/lab_stor5/dongnie/pelvic/models/SegOurData_2p5D_geneDice_mce_lr2e3_lrdceLimit_viewExp_ResSeg_DS_SemGuided_fullAD_lambda0p001_0513_netD_100000.pt", type=str, help="prefix of the to-be-saved model name")
parser.add_argument("--prefixPredictedFN", default="confMap/Seg2p5D_semSeg_wdice_wce_viewExp_resEnhance_fullAD0p001_netD_iter10w_0513_ourData", type=str, help="prefix of the to-be-saved predicted filename")
...
def main():
    ...
    if opt.isDeeplySupervised and opt.isSemanticGuidance:
        ...
        netG = ResSegNet_old(opt.in_channels, opt.out_channels, nd=opt.NDim, isRandomConnection=opt.isResidualEnhancement,isSmallDilation=opt.isViewExpansion, isSpatialDropOut=opt.isSpatialDropOut,dropoutRate=opt.dropoutRate)
...
#         print 'sub%d'%ind,'dice1 = ',diceBladder,' dice2= ',diceProstate,' dice3= ',diceRectumm
        ...
        print 'matProb.shape: ',matProb.shape
        ### for Discriminator network
        if opt.resType==2:
            if opt.isNetDInputIncludeSource:
                matConfLabel, matConfProb, _ = testOneSubjectWithTwoModality4DInput(matProb, matFA, matGT, opt.out_channels_netD,
                                                                         opt.input_sz, opt.output_sz, opt.test_step_sz, netD, opt.netDModelPath, resType=opt.resType,nd=opt.NDim)
            else:
            ...
            if opt.isNetDInputIncludeSource:
                matConfLabel,_ = testOneSubjectWithTwoModality4DInput(matProb, matFA, matGT, opt.out_channels_netD, opt.input_sz,opt.output_sz,opt.test_step_sz,netD, opt.netDModelPath,resType=opt.resType, nd = opt.NDim)
            else:
        ...
        # matConfFGProb = sigmoid(matConfFGProb)
