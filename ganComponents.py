...
class UNet(nn.Module):
    def __init__(self, in_channels, out_channels, TModule=None, FModule=False, nd=2):
        ...
        self.last = conv23DUnit(64, out_channels, 1, nd=nd)
...
class HRResUNet_DS(nn.Module):
    def __init__(self, in_channels, out_channels, isRandomConnection=None, isSmallDilation=None, isSpatialDropOut=None,
        ...
        super(HRResUNet_DS, self).__init__()
        # self.imsize = imsize
        print 'come to HRResUNet_DS'
        assert nd == 1 or nd == 2 or nd == 3, 'nd is not correctly specified!!!!, it should be {1,2,3}'
...
old version
'''
class ResSegNet_old(nn.Module):
    def __init__(self, in_channels, out_channels, isRandomConnection=None, isSmallDilation=None, isSpatialDropOut=None,
                 dropoutRate=0.25, nd=2):
        super(ResSegNet_old, self).__init__()
        # self.imsize = imsize
        print 'come to ResSegNet_old'
        assert nd == 1 or nd == 2 or nd == 3, 'nd is not correctly specified!!!!, it should be {1,2,3}'
        ...
        self.conv_block1_64 = conv23D_bn_relu_Unit(in_channels, 64, 3, padding=1, nd=nd)
...
We use small dilation in the middle layers
In this version, I use deep (inter-mediate) supervision mechanism to help training
We use the semantic guidance (SemG) module for shallow layer feature learning in lateral connection
'''
class SemGResSegNet_DS(nn.Module):
    def __init__(self, in_channels, out_channels, isRandomConnection=None, isSmallDilation=None, isSpatialDropOut=None,
                 dropoutRate=0.25, how2norm='BN', nd=2):
        super(SemGResSegNet_DS, self).__init__()
        # self.imsize = imsize
        print 'come to SemGResSegNet_DS'
        assert nd == 1 or nd == 2 or nd == 3, 'nd is not correctly specified!!!!, it should be {1,2,3}'
        ...
        if how2norm=='GN':
            self.left_conv_block1_32 = conv23D_gn_relu_Unit(in_channels, 32, 3, padding=1, nd=nd)
        else:
            self.left_conv_block1_32 = conv23D_bn_relu_Unit(in_channels, 32, 3, padding=1, nd=nd)
        self.left_conv_block32_32 = residualUnit3(32, 32, isDilation=False, isEmptyBranch1=False, how2norm=how2norm, nd=nd)
        self.left_conv_block32_64 = residualUnit3(32, 64, isDilation=False, isEmptyBranch1=False, how2norm=how2norm, nd=nd)
        self.left_conv_block64_128 = residualUnit3(64, 128, isDilation=False, isEmptyBranch1=False, how2norm=how2norm, nd=nd)

        # the residual layers on the smallest resolution
        self.bottom_conv_block128_256 = residualUnit3(128, 256, isDilation=isSmallDilation, isEmptyBranch1=False, how2norm=how2norm, nd=nd)

        self.dropout23d = dropout23DUnit(prob=dropoutRate, nd=nd)

        #semantic-guidance for shallow layer feature learning
        self.sg1 = SemGuidedModule(32*2,reduction=2, nd=nd) #the 1st stage
        self.sg2 = SemGuidedModule(64*2,reduction=2, nd=nd) #the 2nd stage
        self.sg3 = SemGuidedModule(128*2, reduction=2, nd=nd)  # the 3rd stage

        if how2norm == 'GN':
            self.up3 = convTranspose23D_gn_relu_Unit(256, 128, kernel_size=4, stride=2, padding=1, nd=nd)
            self.up2 = convTranspose23D_gn_relu_Unit(128, 64, kernel_size=4, stride=2, padding=1, nd=nd)
            self.up1 = convTranspose23D_gn_relu_Unit(64, 32, kernel_size=4, stride=2, padding=1, nd=nd)
        else:
            self.up3 = convTranspose23D_bn_relu_Unit(256,  128, kernel_size=4, stride=2, padding=1, nd=nd)
            self.up2 = convTranspose23D_bn_relu_Unit(128,  64, kernel_size=4, stride=2, padding=1, nd=nd)
            self.up1 = convTranspose23D_bn_relu_Unit(64,  32, kernel_size=4, stride=2, padding=1, nd=nd)

        if isRandomConnection:
            self.right_conv_block128_128 = LongSKipAddUnit(128, 128, spatial_dropout_rate=0.1, how2norm=how2norm, nd=nd)
            self.right_conv_block64_64 = LongSKipAddUnit(64, 64, spatial_dropout_rate=0.05, how2norm=how2norm, nd=nd)
            self.right_conv_block32_32 = LongSKipAddUnit(32, 32, spatial_dropout_rate=0.01, how2norm=how2norm, nd=nd)
        else:
            self.right_conv_block128_128 = LongSKipAddUnit(128, 128, spatial_dropout_rate=0, how2norm=how2norm, nd=nd)
            self.right_conv_block64_64 = LongSKipAddUnit(64, 64, spatial_dropout_rate=0, how2norm=how2norm, nd=nd)
            self.right_conv_block32_32 = LongSKipAddUnit(32, 32, spatial_dropout_rate=0, how2norm=how2norm, nd=nd)
        ...
        # intermediate_output1: upsampling 2 times: we firstly use plain conv and then use bilinearly upsmapling
        if how2norm=='GN':
            self.conv_path1 =  conv23D_gn_relu_Unit(64, out_channels, 1, nd=nd)
        else:
            self.conv_path1 = conv23D_bn_relu_Unit(64, out_channels, 1, nd=nd)
        if nd == 2:
            self.last_path1 = nn.Upsample(scale_factor=2, mode='bilinear')
        ...
        # intermediate_output2: upsampling 4 times
        if how2norm=='GN':
            self.conv_path2 = conv23D_gn_relu_Unit(128, out_channels, 1, nd=nd)
        else:
            self.conv_path2 = conv23D_bn_relu_Unit(128, out_channels, 1, nd=nd)
        if nd == 2:
            self.last_path2 = nn.Upsample(scale_factor=4, mode='bilinear')
    ...
    def forward(self, x):
        left_block0 = self.left_conv_block1_32(x)
        left_block1 = self.left_conv_block32_32(left_block0)
        pool1 = self.pool1(left_block1)
        if self.isSpatialDropOut:
            pool1 = self.dropout23d(pool1)

        left_block2 = self.left_conv_block32_64(pool1)
        pool2 = self.pool2(left_block2)
        if self.isSpatialDropOut:
            pool2 = self.dropout23d(pool2)

        left_block3 = self.left_conv_block64_128(pool2)
        pool3 = self.pool3(left_block3)
        if self.isSpatialDropOut:
            pool3 = self.dropout23d(pool3)

        bottom_block4 = self.bottom_conv_block128_256(pool3)  # dilation in the smallest resolution

        up3 = self.up3(bottom_block4) #128 channels
        sg_stage3 = self.sg3(left_block3, up3) #shallow: 128 channels
        right_block3 = self.right_conv_block128_128(sg_stage3,up3)#both 128 channles

        up2 = self.up2(right_block3) #64 channels
        sg_stage2 = self.sg2(left_block2,up2)#both 64 channels
        right_block2 = self.right_conv_block64_64(sg_stage2, up2)

        up1 = self.up1(right_block2) #32 channels
        sg_stage1 = self.sg1(left_block1, up1)
        right_block1 = self.right_conv_block32_32(sg_stage1, up1)

        # for path1:2 times upsampling
        conv_path1 = self.conv_path1(up2)

        # for path2: 4 times upsampling
        conv_path2 = self.conv_path2(up3)

        #         return F.log_softmax(self.last(up4))
        return self.last_main(right_block1), self.last_path1(conv_path1), self.last_path2(conv_path2)



'''
Residual Learning for FCN: short-range residual connection and long-range residual connection
We use spatial dropout after each layer group, differently, we mainly add spatial dropout in the higher layers
We use stochastic long-range connection
We use small dilation in the middle layers, we use more dilation layers in the smallest layers
In this version, I use deep (inter-mediate) supervision mechanism to help training
We also include the dilated residual module in the highest resolution's (HR) lateral connection
'''


class DHRResSegNet_DS(nn.Module):
    def __init__(self, in_channels, out_channels, isRandomConnection=None, isSmallDilation=None, isSpatialDropOut=None,
                 dropoutRate=0.25, nd=2):
        super(DHRResSegNet_DS, self).__init__()
        # self.imsize = imsize
        print 'come to DHRResSegNet_DS'
        assert nd == 1 or nd == 2 or nd == 3, 'nd is not correctly specified!!!!, it should be {1,2,3}'
        ...
        # the residual layers on the smallest resolution
        self.conv_block128_128 = residualUnit3(128, 128, isDilation=isSmallDilation, isEmptyBranch1=False, nd=nd)
        self.conv_block128_256 = residualUnit3(128, 256, isDilation=isSmallDilation, isEmptyBranch1=False, nd=nd)
    ...
    def forward(self, x):
        ...
        blockMid1 = self.conv_block128_128(pool3)
        blockMid2 = self.conv_block128_256(blockMid1)  # dilation in the smallest resolution
        ...
        up2 = self.up_block256_128(blockMid2, block3)
        if self.isSpatialDropOut:
            up2 = self.dropout23d(up2)
        up3 = self.up_block128_64(up2, block2)
        if self.isSpatialDropOut:
            up3 = self.dropout23d(up3)
        up4 = self.up_block64_32(up3, hr_block4)
        if self.isSpatialDropOut:
            up4 = self.dropout23d(up4)
...
In this version, I use deep (inter-mediate) supervision mechanism to help training
We also include the dilated residual module in the highest resolution's (HR) lateral connection
A wider version of DHRResSegNet_DS network
'''


class WDHRResSegNet_DS(nn.Module):
    def __init__(self, in_channels, out_channels, isRandomConnection=None, isSmallDilation=None, isSpatialDropOut=None,
                 dropoutRate=0.25, nd=2):
        super(WDHRResSegNet_DS, self).__init__()
        # self.imsize = imsize
        print 'come to WDHRResSegNet_DS'
        assert nd == 1 or nd == 2 or nd == 3, 'nd is not correctly specified!!!!, it should be {1,2,3}'
        ...
        self.conv_block1_32 = conv23D_bn_relu_Unit(in_channels, 64, 3, padding=1, nd=nd)
        self.conv_block32_32 = residualUnit3(64, 64, isDilation=False, isEmptyBranch1=False, nd=nd)
        self.conv_block32_64 = residualUnit3(64, 128, isDilation=False, isEmptyBranch1=False, nd=nd)
        self.conv_block64_128 = residualUnit3(128, 256, isDilation=False, isEmptyBranch1=False, nd=nd)

        # the residual layers on the smallest resolution
        self.conv_block128_128 = residualUnit3(256, 256, isDilation=isSmallDilation, isEmptyBranch1=False, nd=nd)
        self.conv_block128_256 = residualUnit3(256, 512, isDilation=isSmallDilation, isEmptyBranch1=False, nd=nd)

        self.dropout23d = dropout23DUnit(prob=dropoutRate, nd=nd)

        # high-resoluton module for the lateral connection between the largest feature map
        self.hr_block1 = DilatedResUnit(64, 64, kernel_size=3, stride=1, dilation=2, nd=nd)
        self.hr_block2 = DilatedResUnit(64, 64, kernel_size=3, stride=1, dilation=3, nd=nd)
        self.hr_block3 = DilatedResUnit(64, 64, kernel_size=3, stride=1, dilation=4, nd=nd)
        self.hr_block4 = DilatedResUnit(64, 64, kernel_size=3, stride=1, dilation=5, nd=nd)

        if isRandomConnection:
            self.up_block256_128 = ResUpUnit(512, 256, spatial_dropout_rate=0.1, nd=nd)
            self.up_block128_64 = ResUpUnit(256, 128, spatial_dropout_rate=0.05, nd=nd)
            self.up_block64_32 = ResUpUnit(128, 64, spatial_dropout_rate=0.01, nd=nd)
        else:
            self.up_block256_128 = ResUpUnit(512, 256, spatial_dropout_rate=0, nd=nd)
            self.up_block128_64 = ResUpUnit(256, 128, spatial_dropout_rate=0, nd=nd)
            self.up_block64_32 = ResUpUnit(128, 64, spatial_dropout_rate=0, nd=nd)

        # main_output: the original main path
        self.last_main = conv23DUnit(64, out_channels, 1, nd=nd)

        # intermediate_output1: upsampling 2 times: we firstly use plain conv and then use bilinearly upsmapling
        self.conv_path1 = conv23D_bn_relu_Unit(128, out_channels, 1, nd=nd)
        if nd == 2:
            self.last_path1 = nn.Upsample(scale_factor=2, mode='bilinear')
        elif nd == 3:
            self.last_path1 = nn.Upsample(scale_factor=2, mode='trilinear')
        ...
        # intermediate_output2: upsampling 4 times
        self.conv_path2 = conv23D_bn_relu_Unit(256, out_channels, 1, nd=nd)
        if nd == 2:
            self.last_path2 = nn.Upsample(scale_factor=4, mode='bilinear')
        elif nd == 3:
            self.last_path2 = nn.Upsample(scale_factor=4, mode='trilinear')
...
In this version, I use deep (inter-mediate) supervision mechanism to help training
We also include the dilated residual module in the highest resolution's (HR) lateral connection
Also, Deep supervison mechanism is included
Contour aware is also included
'''
class HRResSegContourNet_DS(nn.Module):
    def __init__(self, in_channels, out_channels, isRandomConnection=None, isSmallDilation=None, isSpatialDropOut=None,
                 dropoutRate=0.25, nd=2):
        super(HRResSegContourNet_DS, self).__init__()
        # self.imsize = imsize
        print 'come to HRResSegNet_DS'
        assert nd == 1 or nd == 2 or nd == 3, 'nd is not correctly specified!!!!, it should be {1,2,3}'
        ...
        self.contour_conv1 = residualUnit3(32, 32, nd=nd)
        self.contour_conv2 = residualUnit3(32, 16, nd=nd)
        self.last_contour = nn.Conv2d(16, 2, 1, nd=nd)
    ...
    def forward(self, x):
        ...
        # contour-classification task1
        contour_conv1 = self.contour_conv1(up4)
        contour_conv2 = self.contour_conv2(contour_conv1)

        #         return F.log_softmax(self.last(up4))
        return self.last_main(up4), self.last_path1(conv_path1), self.last_path2(conv_path2), self.last_contour(contour_conv2)
...
class ResSegContourNet(nn.Module):
    def __init__(self, in_channels, out_channels, isRandomConnection=None, isSmallDilation=None, isSpatialDropOut=None, dropoutRate=0.25, nd=2):
        ...
        print 'come to ResSegContourNet'
        assert nd==1 or nd==2 or nd==3, 'nd is not correctly specified!!!!, it should be {1,2,3}'
...
'''
    ...
    In this version, this is no learning in the upsampling stage, and there are totally 3 downsampling
    In this version, we consider the odd shape input
'''
class Discriminator_my23dFCNv3(nn.Module):
    def __init__(self,in_channels, out_channels, negative_slope = 0.2, nd=2):
        super(Discriminator_my23dFCNv3, self).__init__()
        self._in_channels = in_channels
        self._negative_slope = negative_slope

        self.nd = nd
        self.conv1 = conv23DUnit(in_channels, 32, kernel_size=3, stride=1, padding=1, nd=nd)  # 16->16
        self.relu1 = nn.LeakyReLU(self._negative_slope,inplace=True)
        ...
        self.convX = conv23DUnit(128, out_channels, kernel_size=4, stride=2, padding=2, nd=nd)  # 2


    def forward(self,x):
        # downsample stage

        x= self.conv1(x) # 16-> 16
        x = self.relu1(x)
        x= self.conv2(x) # 16-> 8
        x = self.relu2(x)
        x= self.conv3(x) # 8 -> 4
...
'''
    Disriminator Network for the Adversarial Training.
    In this version, this is no learning in the upsampling stage, and there are totally 3 downsampling
    In this version, we consider the even shape input
'''
class Discriminator_my23dFCNv4(nn.Module):
    def __init__(self,in_channels, out_channels, negative_slope = 0.2, nd=2):
        super(Discriminator_my23dFCNv4, self).__init__()
        self._in_channels = in_channels
        self._negative_slope = negative_slope
        ...
        self.conv2 = conv23DUnit(32, 64, kernel_size=4, stride=2, padding=1, nd=nd)  # 8
        self.relu2 = nn.LeakyReLU(self._negative_slope,inplace=True)

        self.conv3 = conv23DUnit(64, 128, kernel_size=4, stride=2, padding=1, nd=nd)  # 4
        self.relu3 = nn.LeakyReLU(self._negative_slope,inplace=True)

        self.convX = conv23DUnit(128, out_channels, kernel_size=4, stride=2, padding=1, nd=nd)  # 2
