...
parser.add_argument("--in_channels_netD", type=int, default=1, help="the input channels for netD?")
...
parser.add_argument("--prefixModelName", default="/home/dongnie/myPytorchCodes/pytorch-SRResNet23D/SegCha_3D_wce_wdice_viewExp_resEnhance_lrdcr_0110_", type=str, help="prefix of the to-be-saved model name")
parser.add_argument("--prefixPredictedFN", default="preSub45_cha_3D_wce_wdice_viewExp_resEhance_lrdcr_0110_", type=str, help="prefix of the to-be-saved predicted filename")
...
########### We'd better use dataloader to load a lot of data,and we also should train several epoches############### 
    ...
    #         print 'size outputG: ',outputG.unsqueeze(1).size()
            ...
            outputD_shape = outputD_real.size()
            #print(inputs.size())
    
            if opt.isNetDFullyConv:
                #train with real data
                #real_label = torch.ones(outputD_shape[0],outputD_shape[2],outputD_shape[3],outputD_shape[4])
                real_label = torch.LongTensor(outputD_shape[0],outputD_shape[2],outputD_shape[3],outputD_shape[4]).fill_(1)
                ...
                #fake_label=torch.zeros(outputD_shape[0],outputD_shape[2],outputD_shape[3],outputD_shape[4])
                fake_label = torch.LongTensor(outputD_shape[0],outputD_shape[2],outputD_shape[3],outputD_shape[4]).fill_(0)
...
#         print 'outputG shape, ',outputG.size()
            ...
            if opt.out_channels_netD==1:
                averProb = outputD.data.cpu().mean()
            else:
                averProb = 1 - outputD.data[0].cpu().mean()
#             print 'prob: ',averProb
